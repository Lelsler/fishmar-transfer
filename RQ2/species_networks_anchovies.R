...
#look at only anchoviess:

trade_stocks = trade_stocks %>% filter(group_name=="anchovies")

#calculate unweighted average stock status per country per year
network_anchovies = trade_stocks %>% group_by(year, iso3) %>% summarise(mean_super = mean(super)) 
#this is a bit backwards but select trade and re-aggregate with mean stock status
anchovies_trade = trade_stocks  %>% select(iso3, imp.iso3, v, year) %>% distinct() %>% group_by(iso3, imp.iso3, year) %>% summarise(sum_trade = sum(v))
network_anchovies = anchovies_trade %>% left_join(network_anchovies, by = c("iso3", "year"))%>% filter(sum_trade >10)

importers = network_anchovies %>% select(year, imp.iso3, sum_trade)
...
network_anchovies_five = network_anchovies %>% left_join(biggest)
network_anchovies_five = network_anchovies_five %>% filter(!is.na(biggest))

years = unique(network_anchovies_five$year)
...
plotT <- function(t) {
  ...
  network_anchovies_t = network_anchovies_five %>% filter(year==t) %>% select(iso3, imp.iso3, sum_trade, mean_super)
  
  edgelist = network_anchovies_t%>%select(iso3, imp.iso3, sum_trade)
  attribute = network_anchovies_t %>% select(iso3, mean_super)%>% distinct()
...
#plotting parameters
pdf('anchovies_networks.pdf',height = 4,width = 4)
