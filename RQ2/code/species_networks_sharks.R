...
trade_stocks = trade_stocks %>% filter(group_name=="shark")

#calculate unweighted average stock status per country per year
network_shark = trade_stocks %>% group_by(year, iso3) %>% summarise(mean_super = mean(super)) 
#this is a bit backwards but select trade and re-aggregate with mean stock status
shark_trade = trade_stocks  %>% select(iso3, imp.iso3, v, year) %>% distinct() %>% group_by(iso3, imp.iso3, year) %>% summarise(sum_trade = sum(v))
network_shark = shark_trade %>% left_join(network_shark, by = c("iso3", "year"))%>% filter(sum_trade >10)

importers = network_shark %>% select(year, imp.iso3, sum_trade)
...
network_shark_five = network_shark %>% left_join(biggest)
network_shark_five = network_shark_five %>% filter(!is.na(biggest))

years = unique(network_shark_five$year)
...
plotT <- function(t) {
  ...
  network_shark_t = network_shark_five %>% filter(year==t) %>% select(iso3, imp.iso3, sum_trade, mean_super)
  
  edgelist = network_shark_t%>%select(iso3, imp.iso3, sum_trade)
  attribute = network_shark_t %>% select(iso3, mean_super)%>% distinct()
  ...
  V(gf)$size <- ifelse(nodeList$type == "only importing", 10, nodeList$mean_super*10)
...
#plotting parameters
pdf('shark_networks.pdf',height = 4,width = 4)
