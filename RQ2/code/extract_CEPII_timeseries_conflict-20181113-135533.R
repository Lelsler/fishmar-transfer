...
comtrade_1995 = read.csv("/Users/mtn1/Downloads/baci92_1998.csv")
...
match_codes_stocks = stock %>% filter(year == 1998) %>% left_join(stocks, by = "stockid") %>% filter(!is.na(Code.HS1992))%>% 
...
df = unique(comtrade_country %>% left_join(match_codes_stocks, by = c("Code.HS1992", "country", "iso3")))


# only codes that have a stock matched
df2 = unique(df %>% filter(!is.na(stockid)))


write.csv(df2, "comtrade_1992_year1998_with_stocks.csv")
