...
#look at only cuttlefishs:

trade_stocks = trade_stocks %>% filter(group_name=="cuttlefish")

#calculate unweighted average stock status per country per year
network_cuttlefish = trade_stocks %>% group_by(year, iso3) %>% summarise(mean_super = mean(super)) 
#this is a bit backwards but select trade and re-aggregate with mean stock status
cuttlefish_trade = trade_stocks  %>% select(iso3, imp.iso3, v, year) %>% distinct() %>% group_by(iso3, imp.iso3, year) %>% summarise(sum_trade = sum(v))
network_cuttlefish = cuttlefish_trade %>% left_join(network_cuttlefish, by = c("iso3", "year"))%>% filter(sum_trade >10)

importers = network_cuttlefish %>% select(year, imp.iso3, sum_trade)
...
network_cuttlefish_five = network_cuttlefish %>% left_join(biggest)
network_cuttlefish_five = network_cuttlefish_five %>% filter(!is.na(biggest))

years = unique(network_cuttlefish_five$year)
...
plotT <- function(t) {
  ...
  network_cuttlefish_t = network_cuttlefish_five %>% filter(year==t) %>% select(iso3, imp.iso3, sum_trade, mean_super)
  
  edgelist = network_cuttlefish_t%>%select(iso3, imp.iso3, sum_trade)
  attribute = network_cuttlefish_t %>% select(iso3, mean_super)%>% distinct()
  ...
  plot(gf, layout = l, edge.width=E(gf)$weight/5000, edge.arrow.size=.2, edge.color = rgb(0,0,0,0.5), line=-0.5,
...
#plotting parameters
pdf('cuttlefish_networks.pdf',height = 4,width = 4)
